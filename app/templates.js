...
module.exports = function() {
	...
	this.reservationTemplate = function(addr, owner_first_name, owner_last_name, user_first_name, user_last_name, item_serial, item_type, status, date) {
		...
		var intro = "Hello "+owner_first_name+" "+owner_last_name+",\n\n"
		          +"This is an automated message from the Silicon Tracker"
		          +" Service.  The following item you reserved has been checked in by "+user_first_name+ " "+user_last_name+" on "+date+":\n\n";
		...
		var mailOptions = {
			from: process.env.EMAIL_USER,
			to: addr,
			subject: "[Si-Tracker] Silicon Tracker Reservation Notification",
			text: intro+order
		};
		transporter.sendMail(mailOptions, function(error, info){
			if(error) {
